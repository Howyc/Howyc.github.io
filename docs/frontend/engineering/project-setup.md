...
如果你想先了解项目的整体架构，可以阅读 [fetch-mcp-demo 项目详解](/projects/fetch-mcp-demo)。
...
```bash
# 使用 Vite 官方脚手架创建项目
npm create vite@latest fetch-mcp-demo
```
...
<!-- 来自 fetch-mcp-demo/tsconfig.json -->
```jsonc
{
...
<!-- 来自 fetch-mcp-demo/tsconfig.app.json -->
```jsonc
{
  "compilerOptions": {
    // 增量编译缓存文件路径，加速后续编译
...
<!-- 来自 fetch-mcp-demo/tsconfig.node.json -->
```jsonc
{
  "compilerOptions": {
    // 增量编译缓存文件路径
...
<!-- 来自 fetch-mcp-demo/src/main.tsx -->
```tsx
...
<!-- 来自 fetch-mcp-demo/package.json -->
```jsonc
{
  "dependencies": {
