...
export async function fetchDatabaseUsers(): Promise<User[]> {
  return unwrap<User[]>(await fetch(`${API_BASE}/db/users`));
}
...
export async function updateUser(id: number, user: Partial<User>): Promise<User> {
  return unwrap<User>(await fetch(`${API_BASE}/db/users/update?id=${id}`, {
    method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(user),
  }));
}

export async function deleteUser(id: number): Promise<void> {
  await unwrap<void>(await fetch(`${API_BASE}/db/users/delete?id=${id}`, { method: 'DELETE' }));
}

export async function getUserById(id: number): Promise<User> {
  return unwrap<User>(await fetch(`${API_BASE}/db/users/detail?id=${id}`));
}

export async function getUsersByCity(city: string): Promise<User[]> {
  return unwrap<User[]>(await fetch(`${API_BASE}/db/users/by-city?city=${encodeURIComponent(city)}`));
}

export async function searchUsersByEmail(email: string): Promise<User[]> {
  return unwrap<User[]>(await fetch(`${API_BASE}/db/users/search?email=${encodeURIComponent(email)}`));
}

export async function getUserStats(): Promise<{ count: number }> {
  const count = await unwrap<number>(await fetch(`${API_BASE}/db/count`));
  return { count: count as unknown as number };
}

export async function getAllCities(): Promise<string[]> {
  return unwrap<string[]>(await fetch(`${API_BASE}/db/cities`));
}
...
export async function fetchDatabasePosts(): Promise<Post[]> {
  return unwrap<Post[]>(await fetch(`${API_BASE}/db/posts`));
}
...
export async function updatePost(id: number, post: Partial<Post>): Promise<Post> {
  return unwrap<Post>(await fetch(`${API_BASE}/db/posts/update7?id=${id}`, {
    method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(post),
  }));
}

export async function deletePost(id: number): Promise<void> {
  await unwrap<void>(await fetch(`${API_BASE}/db/posts/delete?id=${id}`, { method: 'DELETE' }));
}

export async function getPostById(id: number): Promise<Post> {
  return unwrap<Post>(await fetch(`${API_BASE}/db/posts/detail?id=${id}`));
}

export async function getPostsByUserId(userId: number): Promise<Post[]> {
  return unwrap<Post[]>(await fetch(`${API_BASE}/db/posts/by-user?userId=${userId}`));
}

export async function searchPostsByTitle(title: string): Promise<Post[]> {
  return unwrap<Post[]>(await fetch(`${API_BASE}/db/posts/search?title=${encodeURIComponent(title)}`));
}
